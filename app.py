...
@app.route('/board')
def board_list():
    db = pymysql.connect(host='database-1.c1vzbsxi3wac.ap-northeast-2.rds.amazonaws.com', port=3306, db='pbldb',
        ...
        if error is None:
            ...
            if file:
                ...
                cursor.execute(
                    ...
                    (board_id, file.filename, 's3-myboard-files', 'images/' + file.filename, file.content_type)
                )
            db.commit()
            cursor.close()
            db.close()
...
def board_view():
    ...
    db = pymysql.connect(host='database-1.c1vzbsxi3wac.ap-northeast-2.rds.amazonaws.com',port=3306,db='pbldb',passwd=db_password,user='admin')
    curs = db.cursor(pymysql.cursors.DictCursor)
...
def download():
    ...
    if result :
        ...
        print("-------------------------------------------------------------------")
        return send_file(f'static/broken-1.png', mimetype='image/png')
